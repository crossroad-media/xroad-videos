...
- **1.0.8** — Mobile UX. The aligned grid steps 3 → 2 → 1 columns (tablet → phone) via the `--xrv-cols` custom property so media queries can override it. On phones the featured carousel becomes a native, thumb-swipeable **scroll-snap** strip (peeks the next card; arrows/dots hide, no JS translateX) — no fragile carousel controls on touch.
- **1.0.7** — Paged browse grid. `per_page` (default 9) sets how many cards show before a **Load More** button reveals `load_more` (default 3) more — applied to the filtered/sorted set, and resets when filters change. A **Subscribe** button (YouTube glyph) appears under the grid when `subscribe_url` is set (`subscribe_label` customizes the text). Example: `[xroad-videos layout="library" per_page="9" load_more="3" subscribe_url="https://youtube.com/@yourchannel"]`.
