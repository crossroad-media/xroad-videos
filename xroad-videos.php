<?php
/**
 * Plugin Name:       TargetCancer Videos
 * Plugin URI:        https://crossroad.us
 * Description:        Privacy-by-design, click-to-load video gallery for TargetCancer Foundation. A
 ...
 *                     (xroad/videos). Built by Crossroad Media for TargetCancer Foundation.
 * Version:           1.0.0
 ...
 * Text module, a core Shortcode block, or the xroad/videos block, and renders identically. When TCF
 ...
 *     Seed with Kim Sheridan (content owner) so these match TargetCancer's editorial taxonomy.
 * ------------------------------------------------------------------------------------------------- */
...
function xrv_synonym_map() {
	...
	return array(
		...
		'track-tumor-board'      => 'tcf-001 molecular tumor board case study mtb genomic',
		'patient-stories'        => 'patient story survivor caregiver testimonial lived experience',
		'organizational-history' => 'about tcf foundation history mission origin story',
...
function xrv_schema_jsonld( $records, $page_url = '' ) {
	...
	$collection = array(
		...
		'mainEntity' => array(
			'@type'           => 'ItemList',
			'name'            => 'TargetCancer Foundation rare cancer video library',
...
function xrv_render_meta_box( $post ) {
	...
	$row( 'Dedicated page URL (the /ontarget/videos/{slug}/ page this card links to)', '_xrv_dedicated_url', $dedicated, 'e.g. https://targetcancer.org/ontarget/videos/example/', 'url' );
...
function xrv_plugin_row_meta( $links, $file ) {
	if ( $file === plugin_basename( __FILE__ ) ) {
		...
		$links[] = 'Built for TargetCancer Foundation';
	}
	return $links;
}
...
function xrv_plugin_branding_row() {
	echo '<tr class="plugin-update-tr"><td colspan="4" class="plugin-update colspanchange" style="box-shadow:none;padding:0">'
		...
		. '<strong style="color:#342669">Crossroad Media</strong> &nbsp;·&nbsp; Privacy-by-design video gallery for TargetCancer Foundation &nbsp;·&nbsp; '
